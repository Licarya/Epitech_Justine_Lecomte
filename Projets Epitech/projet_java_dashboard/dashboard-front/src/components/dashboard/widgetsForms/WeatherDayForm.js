...
import WeatherDayWidget from '../widgets/WeatherDayWidget';

const WeatherDayForm = ({widgetType, defaultParams, onWidgetAdded, onWidgetUpdated}) => {
  const form = useForm({
    initialValues: {
  ...
  const onFormSumbitValidated = (values) => {
    ...
    if (defaultParams)
      ...
      onWidgetAdded(widgetType, timer, <WeatherDayWidget />, values);
  }
